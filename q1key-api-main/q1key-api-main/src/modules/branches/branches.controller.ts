...
export class BranchesController {
  ...
  create(@Body() createBranchDto: CreateBranchDto, @CurrentUser() user: any) {
    ...
    if (user.role.roleName === 'Institution' && user.institutionId !== createBranchDto.institutionId) {
      throw new HttpException('You can only create branches in your institution', HttpStatus.FORBIDDEN);
    }
  ...
  findAll(
    ...
    // If Institution owner, only show their branches
    const filterInstitutionId = user?.role?.roleName === 'Institution' 
      ? user.institutionId 
