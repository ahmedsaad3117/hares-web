...
describe("LoansService", () => {
  ...
  describe("create", () => {
    ...
    it("should throw BadRequestException when branch is at capacity", async () => {
      ...
      // Act & Assert
      await expect(
        service.create(createLoanDto, { userId: 1 }),
      ).rejects.toThrow(BadRequestException);
      await expect(
        service.create(createLoanDto, { userId: 1 }),
      ).rejects.toThrow("Branch has insufficient loan capacity");
    ...
    it("should throw NotFoundException when branch does not exist", async () => {
      ...
      await expect(
        service.create(createLoanDto, { userId: 1 }),
      ).rejects.toThrow(`Branch with ID ${createLoanDto.branchId} not found`);
    });
    ...
    it("should throw NotFoundException when product does not exist", async () => {
      ...
      await expect(
        service.create(createLoanDto, { userId: 1 }),
      ).rejects.toThrow(`Product with ID ${createLoanDto.productId} not found`);
    });
