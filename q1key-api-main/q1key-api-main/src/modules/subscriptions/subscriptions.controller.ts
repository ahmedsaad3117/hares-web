 ...
 * DTOs
 */
import { IsString, IsNumber, IsOptional, IsBoolean, IsNotEmpty, IsEnum, Min } from 'class-validator';
...
export class SubscriptionsController {
    ...
    async getAllPlans(@Request() req, @Query('includeInactive') includeInactive?: string) {
        const roleName = getUserRoleName(req.user);
        const showInactive = includeInactive === 'true' && roleName === 'Super Admin';
    ...
    @Put('plans/:id/toggle')
    @UseGuards(JwtAuthGuard)
    async togglePlanVisibility(@Request() req, @Param('id', ParseIntPipe) id: number) {
        if (getUserRoleName(req.user) !== 'Super Admin') {
            throw new HttpException('غير مصرح لك بهذا الإجراء', HttpStatus.FORBIDDEN);
        }
    ...
    async getAllRequests(
        ...
        return this.subscriptionsService.getAllRequests(status, page || 1, limit || 20);
    }
