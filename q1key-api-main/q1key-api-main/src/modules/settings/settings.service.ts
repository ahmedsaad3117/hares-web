...
export class SettingsService {
  ...
  async getSupportInfo() {
    const whatsapp = await this.settingsRepository.findOne({
      where: { key: "support.whatsapp" },
    });
    const email = await this.settingsRepository.findOne({
  ...
  async updateSupportInfo(whatsapp: string, email: string) {
    // Upsert WhatsApp
    let waSetting = await this.settingsRepository.findOne({
      where: { key: "support.whatsapp" },
    });
    if (!waSetting) {
      waSetting = this.settingsRepository.create({
    ...
    // Upsert Email
    let emailSetting = await this.settingsRepository.findOne({
      where: { key: "support.email" },
    });
    if (!emailSetting) {
      emailSetting = this.settingsRepository.create({
        key: "support.email",
